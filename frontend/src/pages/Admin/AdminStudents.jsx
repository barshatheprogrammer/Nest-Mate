...
const AdminStudents = () => {
  ...
  const fetchStudents = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/admin/users?role=student&page=${page}&search=${searchTerm}&status=${filterStatus}`);
  ...
  return (
    <div className="w-full">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500">Manage Students</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/10 text-sm">
                {loading ? (
                  <tr><td colSpan="5" className="p-8 text-center"><div className="animate-spin w-6 h-6 border-2 border-pink-500 border-t-transparent rounded-full mx-auto"></div></td></tr>
                ) : students.length > 0 ? (
                  students.map(student => (
                    <tr key={student._id} className="hover:bg-white/5 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <img src={student.profileImage !== 'default.jpg' ? student.profileImage : `https://ui-avatars.com/api/?name=${encodeURIComponent(student.name)}&background=ec4899&color=fff`} alt={student.name} className="w-10 h-10 rounded-full object-cover" />
                          <div>
                      ...
                      <td className="p-4 text-white/70">{student.college}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-white/50">{new Date(student.createdAt).toLocaleDateString()}</td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {student.status !== 'active' && (
                            <button onClick={() => handleStatusChange(student._id, 'active')} className="p-2 bg-green-500/10 text-green-400 rounded-lg hover:bg-green-500/20 transition tooltip" title="Activate">
                              <CheckCircle size={16} />
                            </button>
                          )}
                          {student.status !== 'suspended' && (
                            <button onClick={() => handleStatusChange(student._id, 'suspended')} className="p-2 bg-yellow-500/10 text-yellow-400 rounded-lg hover:bg-yellow-500/20 transition tooltip" title="Suspend">
                              <Ban size={16} />
                            </button>
                          )}
                          {student.status !== 'blocked' && (
                            <button onClick={() => handleStatusChange(student._id, 'blocked')} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition tooltip" title="Block">
                              <Ban size={16} />
                            </button>
                          )}
                        </div>
